...
import { getMyProfile, updateProfile, updateUsername } from "../../store/slices/user-slice";
					...
					<Box mb="30px" display="flex">
						<Box flex="4">
							<Grid templateColumns={{ md: "repeat(2, 1fr)", sm: "repeat(1, 1fr)" }} gap={6}>
											...
											(
												<Flex width="80%">
													<Input
														...
														flex="1"
													/>
													<Button ml={2} color="green" bg="#e6ebf2" onClick={handleUpdateUsername}>Lưu</Button>
												</Flex>
								...
								<FormControl my={4} isInvalid={profile.email === "" || !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(profile.email)}>
									<Flex alignItems="flex-end">
										<Box width="30%">
								...
								>
									...
									<FormErrorMessage ml={"30%"}>Số điện thoại phải có 10-11 chữ số</FormErrorMessage>
								</FormControl>
								...
								<FormControl my={4} isInvalid={new Date().getFullYear() - new Date(profile.birthday).getFullYear() < 13}>
									<Flex alignItems="flex-end">
										<Box width="30%">
									...
									<FormErrorMessage ml={"30%"}>You must be at least 13 years old</FormErrorMessage>
