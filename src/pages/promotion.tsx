		...
		<Box px={12} py={4}>
			<Box>
				<DashboardPageHeader
					title="Mã vận chuyển"
				/>
				{response && response.data.items.filter(x => x.voucher_type === 1).map((item) =>
			...
			<Box mt={12}>
				<DashboardPageHeader
					title="Mã sản phẩm"
				/>
				{response && response.data.items.filter(x => x.voucher_type === 2).map((item) =>
					<Box borderWidth="1px" borderRadius="lg" padding="6" margin="4">
						<Text fontWeight="bold" marginBottom="2">Code: {item.voucher_code}</Text>
						<Text marginBottom="2">Giảm giá : {item.discount_value.toLocaleString('vi-VN')}₫</Text>
						<Text>Giá tối thiểu để giảm: {item.voucher_require.min_require.toLocaleString('vi-VN')}₫</Text>
						<Text>Chi tiết:</Text>
						<Text>{item.detail}</Text>
