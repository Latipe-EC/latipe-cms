...
import { Box, Button, ButtonGroup, Flex, Input, InputGroup, InputLeftElement, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Select, Spinner, Table, Tbody, Td, Text, Th, Thead, Tr, useToast } from '@chakra-ui/react';
...
const StoresAdmin = () => {
					...
					return (
						<Flex justifyContent={'center'}>
							<ButtonGroup spacing="4">
								<Button colorScheme="green" onClick={() => window.open(`/shop/${item.id}`, '_blank')}>
									Xem chi tiết
								</Button>
							</ButtonGroup>
						</Flex>
	...
	useEffect(() => {
		if (!initialized.current) {
			...
			dispatch(getAdminStore({ skip: currentPage * size, size, keyword: searchText, statusBan: filterBanner }));
		}
	}, []);
	...
	const handleUpdateStatusBan = () => {
		...
		dispatch(updateBanStore({ id: showModalBan.id, isBanned: !showModalBan.isBan, reason: reasonBan })).unwrap().then((res) => {
	...
	const handleFilterChange = (event) => {
		...
		dispatch(getAdminStore({ skip: currentPage * size, size, keyword: searchText, statusBan: event.target.value }));
	}
			...
			{stores.stores.length === 0 ? (
				...
				<FlexBox justifyContent="center" mt="2.5rem" >
					<Pagination
						...
						onChange={(data) => {
							dispatch(getAdminStore({ skip: +data * size, size, keyword: searchText, statusBan: filterBanner }));
							setCurrentPage(+data);
						}}
					/>
				</FlexBox>
