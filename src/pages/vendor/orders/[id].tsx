...
import { OrderItem, StoreOrderDetailResponse } from "api/interface/order";
import { cancelOrderItem, getStoreOrderDetail, updateOrderItemStatusByStore } from "../../../store/slices/orders-slice";
					...
					>
						<Box m="6px">
							<Chip p="0.25rem 1rem" bg={`${getColor(response.data.status)}.light`}>
								<Small textAlign="center" color={`${getColor(response.data.status)}.main`}>{handleRenderStaus()}</Small>
							</Chip>
						</Box>
					</FlexBox>}
					...
					(
						<Grid item lg={6} md={6} xs={12}>
							<Card p="20px 30px" mb="1.5rem">
								...
								<Typography fontSize="14px">{response.data.payment_method === 1 ? " Thanh toán khi nhận hàng" : response.data.payment_method === 2 ?
