...
const AccountSettings = () => {
	...
	const handleProvinceChange = (event: { target: { value: string; }; }) => {
		...
		setStore({
			...store, address: {
				...store.address,
				cityOrProvinceName: selectedProvince.name,
				wardId: null,
				districtId: null,
				wardName: '',
				districtName: '',
				cityOrProvinceId: selectedProvince.code
			}
		});
	...
	const handlePhoneChange = (event: { target: { value: string; }; }) => {
		...
		setPhoneError(isValid ? "" : "Điện thoại number must be 10 digits");
	};
					...
					>
						...
						<Box m="6px">
							<Chip p="0.25rem 1rem" bg="green">
								<Small textAlign="center" color="white" fontWeight="bold">Số
									dư: {store.eWallet.toLocaleString('vi-VN')}₫</Small>
							</Chip>
						</Box>
					</Box>
