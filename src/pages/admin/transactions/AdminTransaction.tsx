...
import { Box, Flex, Input, InputGroup, InputLeftElement, Select, Table, Tbody, Td, Text, Th, Thead, Tr } from '@chakra-ui/react';
...
const TransactionsAdmin = () => {
	...
	useEffect(() => {
		if (!initialized.current) {
			...
			dispatch(getPaginatePayment({ skip: currentPage * size, size, keyword: searchText, statusPayment: filterStatus }));
		}
	}, []);
	...
	const handleFilterChange = (event) => {
		...
		dispatch(getPaginatePayment({ skip: currentPage * size, size, keyword: searchText, statusPayment: event.target.value }));
	}
			...
			{transactions.data.length === 0 ? (
				...
				<FlexBox justifyContent="center" mt="2.5rem" >
					<Pagination
						...
						onChange={(data) => {
							dispatch(getPaginatePayment({ skip: +data * size, size, keyword: searchText, statusPayment: filterStatus }));
							setCurrentPage(+data);
						}}
					/>
				</FlexBox>
