...
import { GetOrderDaysResponse, GetProductBestSellerResponse } from '@interfaces/order';
import BarChartOrder from '@components/chart/LineChartOrder';
import { countAllUser } from '@stores/slices/user-slice';
import { countAllProduct } from '@stores/slices/products-slice';
import { countAllStore } from '@stores/slices/stores-slice';
				...
				<Grid item lg={6} md={6} xs={12} style={{ display: 'flex' }}>
					<Card p='20px' alignItems='center' flexDirection='column' w='100%'>
						...
						{responseProduct && responseProduct.data.items &&
							<PieChartListProduct data={responseProduct.data.items} />}
					</Card>
				</Grid>
				<Grid item lg={6} md={6} xs={12} style={{ display: 'flex' }}>
					<Card p='20px' alignItems='center' flexDirection='column' w='100%'>
						<Flex px='25px' mb="8px" justifyContent='space-between' align='center'>
							...
							<Input
								...
								onChange={(e) => {
									setDateOrder(new Date(e.target.value))
								}}
							></Input>
						</Flex>
						{responseOrder && responseOrder.data.items &&
							<BarChartOrder items={responseOrder.data.items} />}
					</Card>
				</Grid>
			</Grid>
		</Box>
